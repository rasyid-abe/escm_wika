...
if(!empty($userdata['pos_id'])){
  $this->db->group_start();
  $this->db->where("current_approver_id",'('.$userdata['employee_id'].')::text',false);
...
if(!empty($search)){
  ...
  $this->db->or_like('LOWER("created_date")::text',$search);
  $this->db->or_like('LOWER("invoice_date")::text',$search);
  $this->db->or_like("LOWER(invoice_notes)",$search);
  $this->db->or_like("LOWER(vendor_name)",$search);
  $this->db->or_like("LOWER(activity)",$search);
  $this->db->group_end();
}

$this->db->where("COALESCE(invoice_status,null) !=",null);

$data['total'] = $this->Contract_m->getPekerjaanInvoiceWO()->num_rows();
...
$this->db->where("COALESCE(invoice_status,null) !=",null);
$rows = $this->Contract_m->getPekerjaanInvoiceWO()->result_array();
