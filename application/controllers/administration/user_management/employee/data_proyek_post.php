...
if(!empty($search)){
  ...
  $this->db->or_like("LOWER(ppm_dept_id)",$search);
  $this->db->or_like("LOWER(ppm_dept_name)",$search);
  $this->db->group_end();
}

$data['total'] = $this->Administration_m->getEmployeeProyek($filtering)->num_rows();
...
$rows = $this->Administration_m->getEmployeeProyek($filtering)->result_array();
