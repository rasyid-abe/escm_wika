<?php 
  ...
  $view = 'administration/template_vpi/penilaian_penyedia_jasa/penilaian_penyedia_jasa_v';
  ...
  if (isset($post['mode']) and $post['mode'] == 'insert') {
    ...
    $check = $this->Administration_m->getTargetdanBobotKompilasiVPI("","jasa")->num_rows();
    ...
    else
    {
      ...
      redirect(site_url('administration/template_vpi/penilaian_penyedia_jasa'));
  ...
  $get_data_bobot = $this->Administration_m->getTargetdanBobotKompilasiVPI('','jasa')->result_array();
  $data['data_bobot'] = [];
  $arrayNew = array();
...
$this->template($view,"Penilaian Penyedia Jasa",$data);
