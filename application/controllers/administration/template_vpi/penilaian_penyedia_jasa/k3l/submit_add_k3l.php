...
if (!$position) {
 ...
 $post= $this->input->post();
  ...
  $userdata = $this->data['userdata'];
    ...
    $last_seq = intval($this->Administration_m->getK3l("","jasa")->row()->ak_seq) + 1;
  ...
  foreach ($post as $key => $value) {
  	...
  	foreach ($value as $key2 => $value2) { 
  		...
  		$input[$key2]['ak_seq'] = $last_seq == 0 ? 1 : $last_seq;
      $input[$key2]['ak_type'] = 'jasa';
  ...
  if ($this->form_validation->run() == FALSE){
    ...
    $this->template_vpi('penilaian_penyedia_jasa/k3l');
