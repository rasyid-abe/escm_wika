...
if (!$position) {
 ...
 $post= $this->input->post();
  ...
  $userdata = $this->data['userdata'];
    ...
    $last_seq = intval($this->Administration_m->getHasilMutuPekerjaan("","barang")->row()->ahm_seq);
  ...
  foreach ($post as $key => $value) {
      ...
      $input[$key2]['ahm_seq'] = $last_seq;
      $last_seq = $last_seq+1 /count($post);
      $input[$key2]['ahm_type'] = 'barang';
  ...
  if ($this->form_validation->run() == FALSE){
    ...
    $this->template_vpi('penilaian_penyedia_barang/hasil_mutu_pekerjaan');
