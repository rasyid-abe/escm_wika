<?php
    $this->db->trans_begin();
            ...
            if (!empty($this->get('no_baop'))) {
                 $this->db->where('bastp_number', $this->get('no_bapb'));
            }
            if (!empty($this->get('no_sppm'))) {
                $this->db->where('sppm_number', $this->get('no_sppm'));
            }
            $this->db->where('bastp_type', 'baop');
            $data_baop = $this->db->get('vw_data_bastp_pmcs')->result_array();
            if (count($data_baop) > 0) {
                
            $data = array(); 
            $no = 0;
            foreach ($data_baop as $key => $value) {
                ...
                $data[$key]['no_baop'] = $value['bastp_number'];
                $data[$key]['no_bap'] = $value['bastp_number'];
                $data[$key]['no_spk'] = $value['spk_code'];
                $data[$key]['no_sppm'] = $value['sppm_number'];
                $data[$key]['kode_departemen']= $value['dep_code'];
                $data[$key]['tanggal_baop']= $value['bastp_date'];
