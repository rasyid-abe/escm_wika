<?php 
 ...
 $post= $this->input->post();
  ...
  $data['vpp_contract_id'] = $post['contract_id_inp'];
...
$this->db->trans_begin();
   ...
   $this->db->select('MAX(vpp_id) as vpp_id');
   $max_id = (intval($this->Vendor_m->getDataPenilaianPelayanan()->row()->vpp_id) + 1);

   $get_id = $this->Vendor_m->insertDataPenilaianPelayanan($data); //insert
  ...
  else
  {
    ...
    //insert jawaban
        ...
        foreach ($input as $key => $value) {
          $new_array = array("vppa_value"=>$value['vppa_value'],"vppa_pertanyaan_id"=>$value['vppa_pertanyaan_id'],"vpp_id"=>$max_id,"created_datetime"=>date('Y-m-d h:i:s'));
          array_push($dataJawaban, $new_array);
        }

        $insertJawaban = $this->Vendor_m->insertJawabanPenilaianPelayanan($dataJawaban);
