<?php 
$view = 'vendor/vpi/monitor_pekerjaan/penilaian_header_v';
$data = array();
...
foreach ($submitted_date as $dt) {
	$dt['vvh_date'] = DateTime::createFromFormat('Ym', $dt['vvh_date']);  
	array_push($data['date_range']['text'], $dt['vvh_date']->format("F Y"));
	array_push($data['date_range']['val'], $dt['vvh_date']->format("Ym"));
}

$this->db->select('vvh_date,vvh_tipe');
$this->db->where('vvh_contract_id', $contract_data['contract_id']);
$this->db->join('vnd_vpi_kompilasi b', 'b.vvh_id = vnd_vpi_header.vvh_id');
$current_data = $this->Vendor_m->getVPIHeader()->row_array();
