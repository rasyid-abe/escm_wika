...
class PrivySAP extends CI_Controller {
    ...
    public function upload_doc($rfqId)
    {
        ...
        // echo "<pre>";
        // print_r(json_decode($getDataUskep['esign_dsp'])->nama);
        // die;
        $path = base_url()."uploads/".$getDataUskep['bakp_filename'];
        ...
        $listTtd = json_decode($getDataUskep['esign_dpkn'])->nip;
        $penilaian_ttd = json_decode($getDataUskep['esign_dpkn'])->nm_kew;
        ...
        //rifan TWD4277
        $this->db->where('nm_peg', $penilaian_ttd);
        $signer = $this->db->get('vw_user_employee_hcis')->row_array();

        $positionDsp[0]["pos_x"] = (int)$pos_x_penilaian;
                $positionDsp[0]["pos_y"] = (int)$pos_y_penilaian;
                $positionDsp[0]["page"] = $page - 1;
                $positionDsp[1]["pos_x"] = (int)$pos_x_penilaian;
                $positionDsp[1]["pos_y"] = (int)$pos_y_penilaian;
                $positionDsp[1]["page"] = $page -1;
        ...
        {
            $data_update = array(
                'bakp_doc_token' => $res->data[0]->doc_token,
                'bakp_is_share' =>1
            );
    ...
    public function sign_in_doc_request($rfqId,$privyId = "")
    {
        ...
        //base64 imaage
        $getDataUskep = $this->db->get_where('uskep_online', ['no_rfq' => $rfqId])->row_array();
        $docToken = $getDataUskep['bakp_doc_token'];
    ...
    public function sign_in_doc_process($rfqId,$otp_code= "46979",$privyId = "")
    {
        ...
        {
            $data_update = array(
                'bakp_status_esign' => json_encode($res->data),
            );
            $this->db->where('no_rfq', $rfqId);
			$this->db->update('uskep_online', $data_update);
        }
