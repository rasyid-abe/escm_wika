...
$input['pr_type_of_plan'] = "rkp_np"; //y
  ...
  if (is_array($value)) {
    ...
    foreach ($value as $key2 => $value2) {
      ...
      if (isset($post['item_jumlah'][$key2]) && !empty($post['item_jumlah'][$key2])) {
        ...
        $input_item[$key2]['ppi_lampiran'] = $post['doc_attachment_inp_smbd_npmcs'][$key2];
...
if ($this->form_validation->run() == FALSE || $error) {
  ...
  if ($act) {
    ...
    $return = $this->Procedure_m->prc_pr_comment_complete($pr_number, $userdata['complete_name'], 1000, $response, $com, $attachment, $last_id, $perencanaan_id, $userdata['employee_id'], $swakelola_inp, "rkp_np", "", "", "");
  ...
  $this->renderMessage("success", site_url("paket_pengadaan/paket_proyek_non_pmcs"));
}
