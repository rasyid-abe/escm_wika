...
$input['pr_jenis_kontrak'] = $post['jenis_kontrak'];
$input['pr_jadwal_pengadaan_awal'] = $post['tgl_mulai_inp'];
$input['pr_tipe_perencanaan'] = $post['tipe_perencanaan'];
...
$input['pr_type_of_plan'] = "rkp_matgis"; //y
    ...
    if (is_array($value)) {
        ...
        foreach ($value as $key2 => $value2) {
            ...
            if (isset($post['item_jumlah'][$key2]) && !empty($post['item_jumlah'][$key2])) {
                ...
                $input_item[$key2]['ppi_lampiran'] = $post['doc_attachment_inp_smbd_matgis'][$key2];
...
if ($this->form_validation->run() == FALSE || $error) {
    ...
    if ($act) {
        ...
        $return = $this->Procedure_m->prc_pr_comment_complete($pr_number, $userdata['complete_name'], 1000, $response, $com, $attachment, $last_id, $perencanaan_id, $userdata['employee_id'], $swakelola_inp, "rkp_matgis", "", "", "");
