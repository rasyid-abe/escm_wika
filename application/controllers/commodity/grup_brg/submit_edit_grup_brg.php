...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){ 
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      $current_code = $this->Commodity_m->getMatGroup($key2)->row()->mat_group_code;
      ...
      $input[$key2]['mat_group_code']=url_title($code,"_",false);
      $input[$key2]['mat_group_name']=$post['name_inp'][$key2];
      $input[$key2]['mat_group_parent']=$parent;
      $input[$key2]['mat_group_status']=null;
...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
   ...
   if($act){
    $this->Comment_m->insertCommodity("","MATERIAL GROUP",$com,0,"Ubah",$key);
  }
}
