...
$id = (isset($get['id']) && !empty($get['id'])) ? $get['id'] : "";
  ...
  if(!empty($search)){
    ...
    $this->db->or_like('("total_cost")::text', str_replace(',', '.', str_replace('.', '', $search)));
  ...
  $data['total'] = $this->Commodity_m->getSrvSmbdPrice()->num_rows();
  ...
  $rows = $this->Commodity_m->getSrvSmbdPrice()->result_array();
...
$status = array("R"=>"warning","A"=>"success","N"=>"default");
  ...
  foreach ($rows as $key => $value) {
    ...
    $rows[$key]["short_description"] = "<a href='".site_url("commodity/detail_barang_sumberdaya/".$rows[$key]['srv_price_id']."")." ' target='_blank'>".$rows[$key]['short_description']."</a>";

    $rows[$key]["total_price"] = inttomoney($rows[$key]["total_cost"]);
  ...
  $rows[$key]['status_name'] = "<span class='label label-$label'>".$value['status_name']."</span>";
    $rows[$key]['is_active'] = ($rows[$key]['is_active'] == 1) ? "Aktif" : "Tidak Aktif";
      if(!empty($filtering) && $filtering == "approval"){
    $rows[$key]['operate'] = site_url("commodity/daftar_pekerjaan/approval_hrg_jasa_smbd/".$rows[$key]["srv_price_id"]);
  }
  }
