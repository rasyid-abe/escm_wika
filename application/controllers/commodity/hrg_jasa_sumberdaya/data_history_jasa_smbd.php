...
$field_order = (isset($get['sort']) && !empty($get['sort'])) ? $get['sort'] : "csh_id";
...
$data['total'] = $this->Commodity_m->getSrvHist($idmat)->num_rows();
...
$this->db->order_by("csh_id", "desc");
$rows = $this->Commodity_m->getSrvHist($idmat)->result_array();
foreach ($rows as $key => $value) {
  $rows[$key]['cost'] = inttomoney($value['total_price']); 
}
