...
if(!empty($id)){
  ...
  //hlmifzi
  if(strlen($id) == 14){
      $this->session->set_userdata("code_group",substr($id, 0,4));//8
  } elseif (strlen($id) == 15) {
      $this->session->set_userdata("code_group",substr($id, 0,4));//9
  } elseif (strlen($id) == 16) {
      $this->session->set_userdata("code_group",substr($id, 0,4));//10
  }
...
if(!empty($last_item) && empty($id)){
  $this->db->like("code_group",$last_item);//where
}
