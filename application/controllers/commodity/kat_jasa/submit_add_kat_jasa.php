...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      //$this->form_validation->set_rules("code_inp[$key2]", "lang:code #$key2", 'required|is_unique[com_srv_catalog.srv_catalog_code]|max_length['.DEFAULT_MAXLENGTH_CODE.']');
      ...
      //$input[$key2]['srv_catalog_code']=url_title($post['code_inp'][$key2],"_",false);
      $input[$key2]['srv_group_code']=$post['group_level4_inp'][$key2];
...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
   $com = $value['comment'];
   unset($value['comment']);
   $act = $this->Commodity_m->insertDataSrvCatalog($value);
   if($act){
    $this->Comment_m->insertCommodity($act,"SERVICE CATALOG",$com,0,"Buat");
  }
}
