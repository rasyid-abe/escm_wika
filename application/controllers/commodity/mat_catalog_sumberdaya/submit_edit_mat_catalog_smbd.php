...
foreach ($post as $key => $value) {
  ...
  foreach ($value as $key2 => $value2) { 
	...
	$this->form_validation->set_rules("uom_inp[$key2]", "lang:satuan #$key2", 'required');
    ...
    if($post['group_level3_inp'][$key2] != substr($post['code_inp'][$key2],0,strlen($post['group_level3_inp'][$key2]))){
...
if ($this->form_validation->run() == FALSE){
  ...
  // redirect(site_url("commodity/katalog/katalog_barang_sumberdaya/edit"));
