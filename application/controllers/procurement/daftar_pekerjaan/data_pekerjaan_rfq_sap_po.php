...
$data['total'] = $this->Procrfq_m->getPekerjaanRFQSAP($id,$userdata['employee_id'],"")->num_rows();
...
$this->db->select("ptc_id,ptm_number,ptm_requester_name,ptm_packet,ptm_subject_of_work,jenis_pengadaan,activity, waktu,ctr_po_number,ctr_generate_text_number,ctr_is_sync_po_number,vendor_name");
$this->db->where("ptm_status < ", 1901);
$this->db->where("ptm_status != ", 1800);
$this->db->where('ctr_po_number !=', null);

$rows = $this->Procrfq_m->getPekerjaanRFQSAP_PO($id,$userdata['employee_id'],"")->result_array();
