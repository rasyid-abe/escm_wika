...
$view = "procurement/doc_cetak/pdf_penunjuk_penyedia_v";
...
$this->db->where('vendor_id', $penyedia['ptv_vendor_code']);

$vendor = $this->db->get('vnd_header')->row_array();
...
$data=array(
	...
	'manajer_proyek' => $listPeople[count($listPeople) - 1],
...
file_put_contents('uploads/surat_penunjuk/'.$filename, $output);


//update pemenang
$updatePemenang['filename_generate_penunjuk'] = $filename;
$updatePemenang['is_generated_penunjuk'] = '1';
$this->db->where('rfq_no', $rfq_id);
$this->db->update('prc_doc_pengumuman', $updatePemenang);


$name_doc = "BAKP";
$full_url = base_url()."uploads/surat_penunjuk/".$filename;
redirect($full_url);
