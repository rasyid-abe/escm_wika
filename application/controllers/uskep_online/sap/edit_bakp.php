<?php
	...
	$view = 'uskep_online/edit_bakp_sap_v';

	$row = $this->db->get_where('uskep_online', ['no_rfq' => trim($rfqcode)])->row_array();
	$r_dsp = json_decode($row['data_dsp']);
	$r_dpkn = json_decode($row['data_dpkn']);
	$r_bakp = json_decode($row['data_bakp']);

	$twin = $row['win_type'];
	$esign_bakp = json_decode($row['esign_bakp']);
	...
	$data['val_esign'] = json_decode($row['esign_bakp']);
	// $data['esign'] = $esign_bakp;
	// $data['title'] = $this->db->get_where('adm_matriks_kegiatan', ['id'=>$esign_bakp->keg_id])->row_array();
	...
	$data['mtode'] = $row['metode_pengadaan'];
