...
if ($penawaran != '') {
    ...
    if (count($penawaran) !== $nptex) {
        ...
        foreach ($penawaran as $key => $value) {
            ...
            $arr_pen['volume'] = $this->intClean($this->input->post('vol_pen')[$key]);
            $arr_pen['harga_satuan'] = $this->intClean($this->input->post('hrg_sat_pen')[$key]);
            $arr_pen['total_harga'] = $this->input->post('harga_pen')[$key];

            $arr_pen_sat = [];
            foreach ($vend as $k => $val) {
                $arr_pen_sat[] = $this->intClean($this->input->post('hrg_sat_pen_vend_'.$k)[$key]);
            }
...
if ($nego != '') {
    ...
    if (count($nego) !== $nneg) {
        ...
        foreach ($penawaran as $key => $value) {
            ...
            $arr_neg['volume'] = $this->intClean($this->input->post('vol_pen')[$key]);
            $arr_neg['harga_satuan'] = $this->intClean($this->input->post('hrg_sat_pen')[$key]);
            $arr_neg['total_harga'] = $this->input->post('harga_pen')[$key];

            $arr_nego_sat = [];
            foreach ($vend as $k => $val) {
                $arr_nego_sat[] = $this->intClean($this->input->post('hrg_sat_nego_vend_'.$k)[$key]);
            }
...
$arr_dpkn['tipe_proyek'] = $this->input->post('tipe_plan');
$arr_dpkn['catmanagement'] = $this->input->post('catmanagement');
$arr_dpkn['jnspengadaan'] = $this->input->post('jnspengadaan');
$arr_dpkn['nilai'] = $this->input->post('tipeProyek');
...
if ($error) {
    ...
    $send = [$proyek, $pengadaan, $rfq, $vendor, $arr_dpkn['total_negosiai_vendor'], $pss['total_rbap']];
