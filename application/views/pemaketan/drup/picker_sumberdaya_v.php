...
<script type="text/javascript">
  ...
  $(function() {
    ...
    $table.bootstrapTable({
      ...
      url: "<?php echo site_url('paket_pengadaan/get_item_perencanaan_drup') ?>",
    ...
    function totals(value, row, index) {
      let totale = row.volume * row.harga_satuan;
      return totale;
    }
    setTimeout(function() {
  ...
  function getIdSelections() {
    return $.map($table.bootstrapTable('getSelections'), function(row) {
      return row.kode_sumber_daya
    });
  }

  function responseHandler(res) {
    $.each(res.rows, function(i, row) {
      row.state = $.inArray(row.kode_sumber_daya, selections) !== -1;
    });
    return res;
  }
