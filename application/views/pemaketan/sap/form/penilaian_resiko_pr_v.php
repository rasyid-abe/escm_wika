    ...
    <table class="table table-striped" id="category">
      ...
      foreach ($skala_nilai_jasa as $key => $value) {
        ...
        <tr id="value">
          <td>
            <?php echo $myId ?>
            <input type="hidden" name="id_risiko[<?php echo $myId ?>]" value="<?php echo $value['id'] ?>">
          </td>
          <td>
            <?php echo $value['kategori'] ?>
            <input type="hidden" name="kategori_resiko_jasa[<?php echo $myId ?>]" value="<?php echo $value['kategori'] ?>">
          </td>
          <td>
            ...
            <div class='row'>
              ...
              ?>
                <div class='col-sm-2 border-bottom py-1 childed'><?php echo $val['nilai'] ?></div>
          ...
          <td id="nilai_skala">
            <select name="nilai_skala_resiko[<?php echo $myId ?>]" id="nilai_skala_resiko" class="form-control form-control-sm" data-id="<?php echo $myId ?>">
          ...
          <td id="skala_bobot" class="bobot-<?php echo $myId ?>">
            <?php echo $value['bobot'] ?>
            <input type="hidden" name="bobot_jasa[<?php echo $myId ?>]" value="<?php echo $value['bobot'] ?>">
          </td>
          <td id="nilaixbobot">
            <div class="sum-<?php echo $myId ?>">0</div>
            <input class="nilaixbobotto<?php echo $myId ?>" type="hidden" name="nilai_x_bobot[<?php echo $myId ?>]" value="">
          </td>
        </tr>
      <?php } ?>
      <tr>
    ...
    <table class="table table-striped" id="category_barang">
      ...
      foreach ($skala_nilai as $key => $value) {
        ...
        <tr id="value">
          <td>
            <?php echo $myId ?>
            <input type="hidden" name="id_risiko_barang[<?php echo $myId ?>]" value="<?php echo $value['id'] ?>">
          </td>
          <td>
            <?php echo $value['kategori'] ?>
            <input type="hidden" name="kategori_resiko_jasa_barang[<?php echo $myId ?>]" value="<?php echo $value['kategori'] ?>">
          </td>
          <td>
            <button type="button" id="skalaResikoId" class="btn btn-info btn-sm rounded" data-toggle="popover" data-html=true data-placement="left" data-title="
            <div class='row' style='background-color: #2AACE3;'>
            ...
            <div class='row'>
              <?php foreach ($skala_resiko_nilai as $k => $val) {
                if ($val['id_nilai_resiko'] === $value['id']) {
              ?>
                <div class='col-sm-2 border-bottom py-1 childed d-flex align-items-center justify-content-center'><?php echo $val['nilai'] ?></div>
                  <div class='col-sm-10 border-bottom py-1 childed'>
                  <?php
                  $skalaResiko = explode("- ", $val['skala_resiko']);
                  foreach ($skalaResiko as $nilai) {
          ...
          <td id="nilai_skala_barang">
            <select name="nilai_skala_resiko_barang[<?php echo $myId ?>]" id="nilai_skala_resiko_barang" class="form-control form-control-sm" data-id="<?php echo $myId ?>">
              <option selected value="0">Pilih nilai</option>
              <option value="1">1</option>
              <option value="2">2</option>
              <option value="3">3</option>
          ...
          <td id="skala_bobot_barang" class="bobot_barang-<?php echo $myId ?>">
            <?php echo $value['bobot'] ?>
            <input type="hidden" name="bobot_jasa_barang[<?php echo $myId ?>]" value="<?php echo $value['bobot'] ?>">
          </td>
          <td id="nilaixbobot_barang">
            <div class="sum_barang-<?php echo $myId ?>">0</div>
            <input class="nilaixbobotto_barang<?php echo $myId ?>" type="hidden" name="nilai_x_bobot_barang[<?php echo $myId ?>]" value="">
          </td>
        </tr>
      <?php } ?>
      <tr>
...
<script>
  ...
  dataSumTable.each(function() {
    varSumBobot += getnum($(this).find("td:eq(4)").text());
    tempSumBobot.push(getnum($(this).find("td:eq(4)").text()));
  });

  $("#sumBobot").text(varSumBobot);

  dataSumTableBarang.each(function() {
    varSumBobotBarang += getnum($(this).find("td:eq(4)").text());
    tempSumBobotBarang.push(getnum($(this).find("td:eq(4)").text()));
  });
  ...
  $('table#category select[id=nilai_skala_resiko]').change(function(el) {
    ...
    totalValuesSum = 0;
    dataSumTable.each(function() {
      totalValuesSum += getnum($(this).find("td:eq(5)").text());
    });
  ...
  $('table#category_barang select[id=nilai_skala_resiko_barang]').change(function(el) {
    ...
    dataSumTableBarang.each(function() {
      totalValuesSumBarang += getnum($(this).find("td:eq(5)").text());
    });
