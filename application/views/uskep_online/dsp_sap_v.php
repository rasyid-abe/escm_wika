...
<script>
    ...
    function add_teknis_lvl1(vendor) {
        ...
        // let r = $('.aspek_tek_head').length;
        // rr = r + 1;

        // let s = $('.head_tek_sub_' + r).length;
        // ss = s + 1;

        var p = $(".aspek_teknis").length;
        var un = parseInt(p) + 1

        var html = `
            <tr id="aspek_teknis_${uniq}" class="aspek_teknis">
                <th style='background-color: lightgray;'class="text-center">
                    <span class="btn-sm btn btn-danger" onclick="remove_tekn(aspek_teknis_${uniq}, ${vendor}, '${uniq}');"><i class="ft ft-trash"></i></span>
                    <span class="ml-2 btn-sm btn btn-info" onclick="add_teknis_point(${vendor}, '${uniq}');"> <i class="ft ft-plus"></i>
                </span></th>
        ...
        $("#row_tek").after(html);
        // $("#aspek_teknis_" + uniq).after('<tbody id="aspek_tek_'+rr+'" class="aspek_tek_head"></tbody>')
    }

    function add_teknis_point(vendor, aun) {
        // close form harga
        ...
        var html = `
            <tr id="point_teknis_${uniq}" class="point_teknis rmpt_${aun}">
