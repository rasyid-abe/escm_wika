                                ...
                                <td width="100%">
                                    <select class="form-control select2" name="vendor" class="vend_val">
                                        ...
                                        <?php foreach($bidderList as $key => $val){ ?>
                                            <option value='<?= $val['vendor_name'] .'>'. $val['vendor_id'] .'>'. $val['address_street'] .'>'. $val['contact_phone_no'] .'>'. $val['address_phone_no']; ?>'><?= $val['vendor_name']; ?></option>
                                        <?php } ?>
                                    </select>
                                </td>
...
<script type="text/javascript">
    ...
    function set_penawaran(data, vendor) {
        ...
        for (var i = 0; i < parseInt(vendor); i++) {
            ...
            <td>
                <input type="text" class="form-control vol_form ff" name="tkdn_${i}[]" id="tkdn_${i}${uniq}" value="">
            </td>
            <td>
                <div class="btn btn-sm btn-info">Doc</div>
            </td>
            `
            nego += `
            ...
            <td>
                <input type="text" class="form-control vol_form ff" name="tkdnnego_${i}[]" id="tkdnnego_${i}${uniq}" value="">
            </td>
            <td>
                <div class="btn btn-sm btn-info">Doc</div>
            </td>
    ...
    $('#proses_tabel').on('click', function() {
        ...
        $("select[name=vendor]").each(function(){
            if ($(this).val() != '') {
                ...
                vname += `<th style="background-color: #29a7de; color:white; font-weight: bold;" rowspan="3" colspan="4">${vd[0]}</th>`
                addven += `
                <input type="hidden" name="vendor_list[]" value="${vd[1]}">
                <td colspan="4">${vd[2] != 'null' ? vd[2] : '-'}</td>
                `
                hpven += `<td colspan="4">${vd[3] != 'null' ? vd[3] : '-'}</td>`
                telven += `<td colspan="4">${vd[4] != 'null' ? vd[4] : '-'}</td>`
                penven += `<td colspan="4"><input type="text" class="form-control ff" name="penawaran_tgl[]" value=""></td>`
                klaven += `<td colspan="4"><input type="text" class="form-control ff" name="klarifikasi_nego[]" value=""></td>`
                vpenaw += `<td>H. Satuan</td><td class="text-right">Harga</td><td>TKDN</td><td>Dokumen</td>`
                vpento += `<input type="hidden" class="chpr_inp" name="total_ppen[]" value="" id="total_ppen${ii}" />
                <th colspan="2" id="tot_pen_vend${ii}" class="text-right chpr_txt"></th><th colspan="2"></th>`
                vnegot += `<input type="hidden" class="chpr_inp tn_val" name="total_ppeneg[]" value="" id="total_ppeneg${ii}" />
                <th colspan="2" id="tot_nego_vend${ii}" class="text-right chpr_txt"></th><th colspan="2"></th>`
                klartx += `<td colspan="4"><input type="text" class="form-control ff" name="klar_per_vend${ii}[]" value=""></td>`
                ii++
            }
        });
        ...
        var depkn_tbl = `
            <table style="width: 100%" class="table-striped" id="tbl_dpkn">
                <tr>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="7" rowspan="2"></th>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="${4 * span_c}" class="text-center">PENYEDIA <span class=" m-2 btn-sm btn btn-warning" onclick="form_reload();"><i class="ft ft-edit"></i></span></th>
                </tr>
                <tr>
                ...
                <tr>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="2">Paket Pengadaan</th>
                ...
                <tr>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="2">Proyek</th>
                ...
                <tr>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="7"></th>
                    <th style="background-color: #29a7de; color:white; font-weight: bold;" colspan="${4 * span_c}"></th>
                </tr>
                <tr class="bg-blue">
                    <th>1</th>
                    <th colspan="6">DATA PENYEDIA</th>
                    <th colspan="${4 * span_c}"></th>
                </tr>
                <tr>
                    <td>1.1</td>
                ...
                <tr><th colspan="7"></th><th colspan="${4 * span_c}"></th></tr>
                <tr>
                    <th>2</th>
                    <th colspan="3">DATA PEKERJAAN / SPESIFIKASI <button data-id="kode_item" id="item_int_btn" class="ml-2 btn-sm btn btn-info picker sumberdaya_btn integrated hidden"><i class="ft ft-plus"></i></button></th>
                ...
                <tr>
                    <th>3</th>
                    <th colspan="3">KLARIFIKASI <span class="ml-2 btn-sm btn btn-info" onclick="add_klarifikasi(${span_c})"><i class="ft ft-plus"></i></span></th>
                    <th colspan="3"></th>
                    <th colspan="${4 * span_c}"></th>
                </tr>
                <tr>
                    <td>3.1</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="Lingkup Pekerjaan" required></td>
                    ${klartx}
                </tr>
                <tr>
                    <td>3.2</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="Jadwal Pelaksanaan" required></td>
                    ${klartx}
                </tr>
                <tr>
                    <td>3.3</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="Cara Pembayaran" required></td>
                    ${klartx}
                </tr>
                <tr>
                    <td>3.4</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="Jaminan-jaminan" required></td>
                    ${klartx}
                </tr>
                <tr>
                    <td>3.5</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="Denda dan Sangsi" required></td>
                    ${klartx}
                </tr>
                <tr>
                    <td>3.6</td>
                    <td colspan="6"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="SMK3L" required></td>
                    ${klartx}
                </tr>
            ...
            <table style="width: 100%" class="table-striped" id="tbl_esign" border="1">
                <tr>
                    <th colspan="3" class="ml-3">
                        ...
                        E-Sign Kewenangan
                        <select style="width: 200px" class="ml-2 form_header" onchange="changetipeplan()" id="tipePlan" name="tipe_plan">
                            <option value="">Tipe Plan</option>
                            <option value="proyek">PROYEK</option>
                            <option value="non-proyek">NON PROYEK</option>
                        </select>
                        <select style="width: 200px" class="ml-2 form_header hidden" onchange="changecatmanagement()" id="catmanagement" name="catmanagement">
                            <option value="">Kategori Management</option>
                            <option value="1">YA</option>
                            <option value="0">TIDAK</option>
                        </select>
                        <select style="width: 200px" class="ml-2 form_header hidden" onchange="changejnspengadaan()" id="jnspengadaan" name="jnspengadaan">
                            <option value="">Jenis Pengadaan</option>
                            <option value="oa">OA</option>
                            <option value="non-oa">NON-OA</option>
                        </select>
                        <select style="width: 200px" class="ml-2 form_header hidden" onchange="changetipeproyek('')" id="tipeProyek" name="tipe_proyek">
                            <option value="">Tipe Proyek</option>
                            <option value="kecil">KECIL</option>
                            <option value="menengah">MENENGAH</option>
                            <option value="besar">BESAR / MEGA</option>
                        </select>
                    </th>
                </tr>
    ...
    function add_catatan() {
        ...
        <td colspan="6"><input type="text" class="form-control ff" name="note[]" value=""></td>
        <td colspan="4"></td>
        </tr>
        `
    ...
    function add_klarifikasi(vendor) {
        ...
        <tr id="klar_${uniq}">
            ...
            <td colspan="3"><input type="text" class="form-control ff" name="poin_klarifikasi[]" value="" required></td>
            <td colspan="3"><input type="text" class="form-control ff" name="klar_rabp[]" value="" required></td>
            ${elem}
        </tr>
        `
    ...
    //     if ($('#idprk').val() != '') {
    //         $.ajax({
    //             url: '<?php echo site_url("uskep_online_sap/get_klasifikasi/");?>' + $('#idprk').val(),
    //             type: 'get',
    //             dataType: 'json',
    //             async: false,
    //             success: function(data) {
    //                 klasifikasi = data['klasifikasi_name'];
    //                 if (klasifikasi == 'Mega' || klasifikasi == 'Besar') {
    //                     klasifikasi = 'besar';
    //                 } else {
    //                     klasifikasi = data['klasifikasi_name'];
    //                 }
