        ...
        <div class="wrapper-header">
          ...
          <div class="wrapper-button" id="btnLampiranProcurement" style="display: none;">
            <span class="wrapper-action save-comment">
              ...
              <input type="hidden" id="current_id_lampiran_cr" value="" />
            </span>
          </div>
        </div>
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    <td>
                      <?= $no ?>
                      <input type="hidden" class="doc_id_inp" value="<?= $v['ppd_id'] ?>" name="doc_id_inp[<?= $no ?>]" data-no="<?= $no ?>">
                    </td>
                    <td class='text-left'>
                      <input type="hidden" class="doc_category_inp" value="<?= $v['ppd_category'] ?>" name="doc_category_inp[<?= $no ?>]" data-no="<?= $no ?>">
                      <?= $v['ppd_category'] ?>
                    </td>
                    <td class="text-left">
                      <input type="hidden" class="doc_attachment_inp" value="<?= $v['ppd_file_name']; ?>" name="doc_attachment_inp[<?= $no ?>]" data-no="<?= $no ?>">
                      <a href="<?= base_url("/uploads/pemaketan/") . $v['ppd_file_name'] ?>"><?= $v['ppd_file_name'] ?></a>
                    </td>
                    <td class="text-left">
                      <input type="hidden" class="doc_desc_inp" value="<?= $v['ppd_description'] ?>" name="doc_desc_inp[<?= $no ?>]" data-no="<?= $no ?>"><?= $v['ppd_description'] ?>
                    </td>
                    <td>
                      <button type='button' class='btn btn-info btn-sm edit_dok_pr_cr' data-no="<?= $no ?>"><i class='fa fa-edit'></i></button>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(document.body).on("click", ".edit_dok_pr_cr", function() {
      ...
      let category = $(".doc_category_inp[data-no='" + no + "']").val()
      let file = $(".doc_attachment_inp[data-no='" + no + "']").val()
      let desc = $(".doc_desc_inp[data-no='" + no + "']").val()
      let getfile = file.split()
      if ($(".doc_id_inp[data-no='" + no + "']").val()) {
        var id = $(".doc_id_inp[data-no='" + no + "']").val();
        $("#current_id_lampiran_cr").val(id);
      }

      $("#current_dok_lampiran_cr").val(no);
    ...
    $(".action_save_document").click(function() {
      ...
      if (doc_category_inp == "" || doc_category_inp == null || doc_attachment_inp == "" || doc_desc_inp == "") {
        ...
        let id_doc = "";
        if ($('#current_id_lampiran_cr').val()) {
          id_doc = $('#current_id_lampiran_cr').val();
        }

        var html = "<tr>";
        html += "<td><input type='hidden' class='doc_id_inp' data-no='" + no + "' name='doc_id_inp[" + no + "]' value='" + id_doc + "' />" + no + "</td>";
        html += "<td class='text-left'><input type='hidden' class='doc_category_inp' value=" + JSON.stringify(doc_category_inp) + " name='doc_category_inp[" + no + "]' data-no='" + no + "'>" + doc_category_inp + "</td>";
        html += "<td class='text-left'><input type='hidden' class='doc_attachment_inp' value=" + doc_attachment_inp + " name='doc_attachment_inp[" + no + "]' data-no='" + no + "'>" +
          "<a href='" + document.location.origin + '/log/download_attachment/procurement/tender/' + doc_attachment_inp + "'>" + doc_attachment_inp + "</a></td>";
        html += "<td class='text-left'><input type='hidden' class='doc_desc_inp' value=" + JSON.stringify(doc_desc_inp) + " name='doc_desc_inp[" + no + "]' data-no='" + no + "'>" + doc_desc_inp + "</td>";
  ...
  function resetFormDoc() {
    $('#current_dok_lampiran_cr').val("");
    $('#current_id_lampiran_cr').val("");
