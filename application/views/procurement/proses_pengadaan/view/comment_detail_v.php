                            ...
                            <div class="col-md-4">
                                <div class="input-group align-items-center">
                                    <span class="input-group-btn">
                                        <button type="button" data-id="prc_lampiran_edit" data-folder="<?php echo $dir ?>" data-preview="preview_file_catatan" class="btn btn-info upload rounded">
                                            <i class="fa fa-cloud-upload"></i> Up
                                        </button>
                                        <button type="button" data-url="<?php echo site_url('log/download_attachment/procurement/tender/' . $curval) ?>" class="btn btn-info preview_upload rounded mr-1" id="preview_file_catatan">
...
<script>
    ...
    function fetchData() {
        ...
        $.ajax({
            url: '<?= site_url($controller_name . "/proses_pengadaan/get_comment_permintaan_pengadaan/" . $uris); ?>',
            method: 'post',
            dataType: 'json',
    ...
    $('.save-submit').click(function() {
        ...
        if (obj_nilai === '' || lampiran === '' || no_telp === '' || comment === '' || respons === '') {
            ...
            if (id_edit == "") {
                <?php $url_save = site_url($controller_name . "/proses_pengadaan/save_comment_permintaan_pengadaan/" . $uris); ?>
                fetch("<?= $url_save ?>", {
                        body: formData,
                        method: "post"
                    })
                ...
                <?php $url_edit = site_url($controller_name . "/proses_pengadaan/edit_comment_permintaan_pengadaan/" . $uris); ?>
    ...
    function onDeleteComment(id) {
        ...
        <?php $url_del = site_url($controller_name . "/proses_pengadaan/delete_comment_permintaan_pengadaan"); ?>
        let fd = new FormData();
        fd.append('id', id);
