...
if($state==0){
  ...
  $where_data    = array('inv_id'=>$id,'cwo_name' => null);
...
if( $state==1 || $state==0){
  //Standard Header
  $inputs = array(
    "inv_number"=>strtoupper($post["inv_number"]),
    ...
    "status"=>$activity_id,
    "inv_total"=>$post['total_alokasi_inp'],
    'bapb_id' =>$post['id'],
  ...
  if($state==0){
    if($this->Contract_m->number_exist('ctr_inv_header',$post['inv_number'])){
      $this->setMessage('inv Number sudah ada');
      $error = true;
    }
  }
  ...
  $old_id=0;
  if($state==0){
    $this->Global_m->insert_table('ctr_inv_header', $inputs);
    $old_id=$id;
    $id = $this->db->insert_id();
  }else{
    $this->Global_m->update_table('ctr_inv_header',$inputs,$id);
  }
  ...
  if ($items) { //check if there's item selected
    ...
    foreach ($items as $key => $value) {
      ...
      if($state==0){
        ...
        $where       = array('inv_item_id' => $key);
        $dt          = $this->Global_m->get_data('ctr_inv_item', $where);
      ...
      if ($state == 1) {
        $this->Global_m->update_table('ctr_inv_item',$input_items, $key);
      } elseif ($state == 0) {
        $this->Global_m->insert_table('ctr_inv_item',$input_items);
      }
    }//end foeach items
  }

  if($state==0){
      $this->Contract_m->duplicate_item_bapb_to_inv($old_id,$id);
  }
  // for ($i=0; $i <=5 ; $i++) {
  ...
  //   }
  // }
  $files = $this->do_upload('attachment', $this->session->userdata("userid"), "inv");
  if (is_array($files))
  {
  }
  else
  ...
  if(isset($filename)){
    if($filename!==""){
      $file_data = array(
        ...
        $file_ex=$this->Global_m->get_data("ctr_inv_doc",array('inv_id' => $id));
        //print_r($file_ex);die;
        if(isset($file_ex)){
          $this->Global_m->update_table("ctr_inv_doc",$file_data,$id);
        }else{
          $this->Global_m->insert_table("ctr_inv_doc",$file_data);
        }
      }
  }
  ...
  $qry='SELECT SUM("qty")*sum("price") AS "total_item" FROM "ctr_inv_item" WHERE "inv_id" = '.$id;
  ...
  switch (strtoupper($response_text)) {
    case "SIMPAN DAN LANJUT INVOICE":
  ...
  if($activity_id==$activity_first){
    if($state==0){
      ...
      $this->Global_m->insert_table("ctr_inv_comment", $comment_arr);
    }
  }else{
      $comment_arr = array(
        "cwo_response"   => $response_text,
      ...
      $comment_id=$last_comment['cwo_id'];
      $this->Global_m->update_table("ctr_inv_comment", $comment_arr, $comment_id);
  ...
  if($state==0 || $state==3){
    ...
    if ($activity_ins == $activity_cancel) {
      ...
      $this->Global_m->update_table("ctr_inv_header", $data, $id);
    }else{
