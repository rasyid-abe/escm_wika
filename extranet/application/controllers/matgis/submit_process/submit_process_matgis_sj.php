...
if($state==0){
  ...
  $where_data    = array('sj_id'=>$id,'cwo_name' => null);
  $last_comment  = $this->Global_m->get_data('ctr_sj_comment', $where_data);
...
if( $state==1 || $state==0){
  //Standard Header
  $inputs = array(
    "sj_number"=>strtoupper($post["sj_number"]),
  ...
  if($state==0){
    if($this->Contract_m->number_exist('ctr_sj_header',$post['sj_number'])){
      $this->setMessage('SJ Number sudah ada');
      $error = true;
    }
  }
  ...
  if($state==0){
    $this->Global_m->insert_table('ctr_sj_header', $inputs);
    $id = $this->db->insert_id();
  }else{
    $this->Global_m->update_table('ctr_sj_header',$inputs,$id);
  }
  ...
  if ($items) { //check if there's item selected
    ...
    foreach ($items as $key => $value) {
      ...
      if ($state == 1) {
        $this->Global_m->update_table('ctr_sj_item',$input_items, $key);
      } elseif ($state == 0) {
        $this->Global_m->insert_table('ctr_sj_item',$input_items);
      }
    }//end foeach items
  }
  // for ($i=0; $i <=5 ; $i++) {
  ...
  //   }
  // }
  $files = $this->do_upload('attachment', $this->session->userdata("userid"), "sj");
  if (is_array($files))
  {
  }
  else
  ...
  if(isset($filename)){
    if($filename!==""){
      $file_data = array(
        ...
        $file_ex=$this->Global_m->get_data("ctr_sj_doc",array('sj_id' => $id));
        //print_r($file_ex);die;
        if(isset($file_ex)){
          $this->Global_m->update_table("ctr_sj_doc",$file_data,$id);
        }else{
          $this->Global_m->insert_table("ctr_sj_doc",$file_data);
        }
      }
  }
  ...
  $qry='SELECT SUM("qty")*sum("price") AS "total_item" FROM "ctr_sj_item" WHERE "sj_id" = '.$id;
  ...
  if($activity_id==$activity_first){
    if($state==0){
      ...
      $this->Global_m->insert_table("ctr_sj_comment", $comment_arr);
    }
  }else{
      $comment_arr = array(
        "cwo_response"   => $response_text,
      ...
      $comment_id=$last_comment['cwo_id'];
      $this->Global_m->update_table("ctr_sj_comment", $comment_arr, $comment_id);
  ...
  if($state==0 || $state==3){
    ...
    if ($activity_ins == $activity_cancel) {
      ...
      $this->Global_m->update_table("ctr_sj_header", $data, $id);
    }else{
