...
if($state==0){
  ...
  $where_data    = array('do_id'=>$id,'cwo_name' => null);
  $last_comment  = $this->Global_m->get_data('ctr_do_comment', $where_data);
...
if( $state==1 || $state==0){
  //Standard Header
  $inputs = array(
    "do_number"=>strtoupper($post["do_number"]),
    ...
    "do_notes"=>$post['do_notes'],
    "status"=>$activity_id,
    "do_total"=>$post['total_alokasi_inp'],
    "sppm_id"=>$post['sppm_id'],
    "do_title"=>$post['do_title'],
  );

  if($state==0){
    if($this->Contract_m->number_exist('ctr_do_header',$post['do_number'])){
      $this->setMessage('DO Number sudah ada');
      $error = true;
    }
  }
  ...
  if($state==0){
    $this->Global_m->insert_table('ctr_do_header', $inputs);
    $id = $this->db->insert_id();
  }else{
    $this->Global_m->update_table('ctr_do_header',$inputs,$id);
  }
  ...
  if ($items) { //check if there's item selected
    ...
    foreach ($items as $key => $value) {
      ...
      if($state==0){
        $where       = array('sppm_item_id' => $key);
        $dt          = $this->Global_m->get_data('ctr_sppm_item', $where);
      }else{
        $where       = array('do_item_id' => $key);
        $dt          = $this->Global_m->get_data('ctr_do_item', $where);
      }
      ...
      if ($state == 1) {
        $this->Global_m->update_table('ctr_do_item',$input_items, $key);
      } elseif ($state == 0) {
        $this->Global_m->insert_table('ctr_do_item',$input_items);
      }
    }//end foeach items
  }
  // for ($i=0; $i <=5 ; $i++) {
  ...
  //   }
  // }
  $files = $this->do_upload('attachment', $this->session->userdata("userid"), "do");
  if (is_array($files))
  {
  }
  else
  ...
  if(isset($filename)){
    if($filename!==""){
      $file_data = array(
        ...
        $file_ex=$this->Global_m->get_data("ctr_do_doc",array('do_id' => $id));
        //print_r($file_ex);die;
        if(isset($file_ex)){
          $this->Global_m->update_table("ctr_do_doc",$file_data,$id);
        }else{
          $this->Global_m->insert_table("ctr_do_doc",$file_data);
        }
      }
  }
  ...
  $qry='SELECT SUM("qty")*sum("price") AS "total_item" FROM "ctr_do_item" WHERE "do_id" = '.$id;
  ...
  switch (strtoupper($response_text)) {
    case "SIMPAN DAN LANJUT DO":
    $activity_ins = 2062; //Pembuatan SKBDN
  ...
  if($activity_id==$activity_first){
    if($state==3 || $state==0){
      ...
      $this->Global_m->insert_table("ctr_do_comment", $comment_arr);
    }
  }else{
      $comment_arr = array(
        "cwo_response"   => $response_text,
      ...
      $comment_id=$last_comment['cwo_id'];
      $this->Global_m->update_table("ctr_do_comment", $comment_arr, $comment_id);
  ...
  if($state==0 || $state==3){
    ...
    if ($activity_ins == $activity_cancel) {
      ...
      $this->Global_m->update_table("ctr_do_header", $data, $id);
    }else{
