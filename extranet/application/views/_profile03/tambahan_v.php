	...
	<form class="form-bordered" method="POST" action="<?php echo site_url('registrasi_vendor/submit_data_utama'); ?>" enctype="multipart/form-data">
								...
								<div class="col-md-9">
									<div class="position-relative">
										...
										<?php if ($detail_vendor['upload_bukti_kontrak'] != NULL) { ?>
											<span>
												<a class="btn bg-light-success btn-sm" href="<?php echo site_url('attachment/vendor/' . $this->session->userdata('npwp_no_s') . '/') . $detail_vendor['upload_bukti_kontrak'] ?>" target="_blank"><i class="ft-download-cloud"></i> Download file</a>
											</span>
										<?php } ?>
									</div>
								</div>
									...
									<tbody>
										<?php $no_bidang = 1;
											...
											<tr>
												...
												<td>
													<button data-no="<?php echo $myid ?>" class="btn btn-info btn-sm edit_bidang" type="button">
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title text-bold-600 float-left">Company Profile <span class="text-danger">(*)</span></h5>
						<a href="javascript:void(0)" class="btn btn-info modified btn-sm float-right" data-toggle="modal" data-target="#companyForm"><i class="fa fa-plus"></i> Tambah</a>
					</div>
					<div class="card-content">
						<div class="card-body">
							<!-- content -->
										...
										<?php $no=1; foreach ($company as $value) { ?>
											<tr>
												...
												<td class="text-center">
													<a href="#" onclick="return confirm('Apakah Anda yakin akan hapus data ini?')" class="btn btn-sm btn-danger"><i class="ft-trash"></i></a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
					...
					<div class="card-content">
						<div class="card-body">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<div class="position-relative">
										<select class="select2 form-control" name="anak_perusahaan" required>
											<option value="" selected disabled>Pilih</option>
											<?php foreach ($anak_perusahaan as $value) { ?>
												<option value="<?php echo $value['vendor_id']; ?>" <?php echo $detail_vendor['anak_perusahaan'] == $value['vendor_id'] ? 'selected' : ''; ?>><?php echo $value['vendor_name']; ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
		...
		<div class="col-lg-12 text-center my-3">
			...
			<?php } ?>
			<a href="<?php echo site_url('registrasi_vendor/catatan'); ?>" onclick="return confirm('Apakah Anda yakin dengan data di atas?')" class="btn btn-info btn-md">Selanjutnya</a>
		</div>
	</form>
</div>
