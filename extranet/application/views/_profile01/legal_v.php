		...
		<form class="form-bordered">
			...
			<div class="row">
				<div class="col-12">
					<div class="card">
						<div class="card-header">
							<h5 class="card-title-w float-left">Izin <span class="text-danger">(*)</span></h5>
							<?php if ($detail_vendor['reg_status_id'] != 8 && $detail_vendor['reg_status_id'] != 14 && $detail_vendor['vnd_jenis'] != 'Pengadaan.com') { ?>
...
<script type="text/javascript">
	...
	$('#upload-izin').change(function() {
		var i = $(this).next('label').clone();
		var file = $('#upload-izin')[0].files[0].name;
		$(this).next('label').text(file);
	});

	$('#upload-izin-edit').change(function() {
		var i = $(this).next('label').clone();
		var file = $('#upload-izin-edit')[0].files[0].name;
		$(this).next('label').text(file);
	});
...
<script>
	...
	$(document).ready(function() {
		$('#editizinForm').on('show.bs.modal', function(event) {
			var div = $(event.relatedTarget)
			var modal = $(this)
